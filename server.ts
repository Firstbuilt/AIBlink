...
function recalculateStats() {
  ...
  riskReport.stats.enforcement.count = updates.length; // Simplified: assuming all updates are relevant actions for now
}
...
app.post("/api/refresh", async (req, res) => {
  try {
    ...
    const searchPrompt = "Latest EU AI regulation news, privacy compliance updates, and enforcement actions from last 3 months. Include specific company names and regulators involved. Find links to sources.";
    ...
    if (newData.newUpdates && Array.isArray(newData.newUpdates)) {
      // Prepend new updates
      updates = [...newData.newUpdates, ...updates];
    }
    if (newData.riskReport) {
      // Preserve stats structure but update content
      riskReport.lastUpdated = newData.riskReport.lastUpdated;
      riskReport.score = newData.riskReport.score;
      riskReport.summary = newData.riskReport.summary;
      riskReport.focusAreas = newData.riskReport.focusAreas;
    }

    // Update Knowledge Base
    if (newData.updatedKnowledgeBaseItems && Array.isArray(newData.updatedKnowledgeBaseItems)) {
       newData.updatedKnowledgeBaseItems.forEach(newItem => {
         const index = knowledgeBase.findIndex(kb => kb.id === newItem.id || kb.title.en === newItem.title.en);
         if (index !== -1) {
           knowledgeBase[index] = { ...knowledgeBase[index], ...newItem };
         } else {
